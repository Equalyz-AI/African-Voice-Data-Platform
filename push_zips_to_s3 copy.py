...
MAX_CONCURRENT_BATCHES = 4  # max batches processed at the same time
# ------------------------------------------------------------------
            ...
            if not ok:
                for i in range(1, 100):
                    ...
                    if ok:
                        ...
                        break
            if not ok:
                logger.warning(f"❌ Not found: {file_to_download}")
                return None
            return local_file_path, arcname, sample
...
async def prezip_dataset_to_main_one(language: str, pct: float = 100, split: Optional[str] = None):
    ...
    async with session_maker() as session:
        samples_stream, _ = await download_service.filter_core_stream(
            session=session,
            language=language,
            pct=pct,
            split=split,
        ...
        logger.info(f"Processing {total_remaining} files in {len(batches)} batches.")
...
if __name__ == "__main__":
    ...
    async def process_combination(language, split):
        logger.info(f"==== Starting: Language={language}, Split={split} ====")
        try:
